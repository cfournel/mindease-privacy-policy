...
## AdMob's app-ads.txt requirement

AdMob/Play Console requires `app-ads.txt` at the root of the domain declared as the
app's website in the Play Console store listing (`onirahypno.com`), listing which ad
...
```
google.com, pub-5304245285512392, DIRECT, f08c47fec0942fa0
```

`pub-5304245285512392` is the publisher ID from the AdMob app ID
(`ca-app-pub-5304245285512392~3677276693`) declared in
[mindease](https://github.com/cfournel/mindease)'s `AndroidManifest.xml` — keep the two
in sync if the AdMob account/app ID ever changes. `f08c47fec0942fa0` is Google's own
fixed certification ID, not account-specific. It's a static file, not generated by
`build.py` (same tier as `CNAME`) since it has nothing to template per language.
